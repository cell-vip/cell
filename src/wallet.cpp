...
bool CWallet::CreateTransactionB(const vector<pair<CScript, int64> >& vecSend,const CTxDestination& address1, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
    int64 nValue = 0;
    BOOST_FOREACH (const PAIRTYPE(CScript, int64)& s, vecSend)
    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // Reserve a new key pair from key pool
                    //ubKey vchPubKey = reservekey.GetReservedKey();
                    // assert(mapKeys.count(vchPubKey));
                    ...
                    // change transaction isn't always pay-to-bitcoin-address
                    CScript scriptChange;
                    scriptChange.SetDestination(address1);
...
bool CWallet::CreateTransactionB(CScript scriptPubKey, const CTxDestination& address1,int64 nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
    vector< pair<CScript, int64> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransactionB(vecSend, address1,wtxNew, reservekey, nFeeRet);
}
...
string CWallet::SendMoneyB(CScript scriptPubKey, const CTxDestination& address1,int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    CReserveKey reservekey(this);
    int64 nFeeRequired;
    ...
    if (!CreateTransactionB(scriptPubKey,address1, nValue, wtxNew, reservekey, nFeeRequired))
    {
        string strError;
        if (nValue + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds  "), FormatMoney(nFeeRequired).c_str());
...
string CWallet::SendMoneyToDestinationB(const CTxDestination& address,const CTxDestination& address1, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
        return _("Invalid amount");
    ...
    return SendMoneyB(scriptPubKey,address1, nValue, wtxNew, fAskFee);
}
